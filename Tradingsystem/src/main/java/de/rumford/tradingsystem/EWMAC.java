...
/**
 * The EWMAC is a non-binary {@link Rule} utilizing the different horizons of its 2 underlying {@link EWMA}. When the
 * shorter horizon EWMA raises above the longer horizon EWMA in value the underlying asset has been rising in the
...
public class EWMAC extends Rule {
	...
	/**
	 * A rule based on the relation between two exponentially weighted moving averages - EWMAs. The EWMAC assumes that
	 * an asset is rising in value, if its short term values are greater than its long term values. If the short horizon
	 * EWMA is greater than the long horizon one, a positive forecast is given. Else, a negative forecast is produced.
	 * <p>
	 * This non-binary rule allows for a base value scale independent detailed forecast calculation. The greater the
	...
	/**
	 * Calculates the raw forecast for a given LocalDateTime by subtracting the long horizon EWMA value from the short
	 ...
	 * 
	 * @param  longHorizon              {@code int} The given long horizon. Must be > shortHorizon.
	 ...
	 * 
	 * @param longHorizonEwma {@link EWMA} long horizon the be set for this {@link EWMAC}
	 */
	 ...
	 * 
	 * @param shortHorizonEwma short horizon {@link EWMA} to be set for this {@link EWMAC}
	 */
