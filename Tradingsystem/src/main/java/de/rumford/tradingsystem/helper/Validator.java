...
/**
 * Validator class as used throughout the library containing solely of static methods representing validation of input
 * values.
 * 
 * @author Max Rumford
 */
public class Validator {
	 ...
	 * 
	 * @param  correlations             {@code double[]} Correlations to be validated. Must not be null. Must have a
	 *                                  length of 3. Must only contain values {@code !Double.NaN} and
	 *                                  {@code -1 <= value <= 1}.
	...
	/**
	 * Validates if the given array of ValueDateTupel is sorted in an ascending order.
	 * 
	...
	/**
	 * Validates the given double value. The value must meet the following specifications:
	 * <ul>
	...
	/**
	 * Validates the given row of values. The row has to have at least 1 value not Double.NaN.
	 * 
	 * @param  valueDateTupels          {@code ValueDateTupel[]} The array of {@link ValueDateTupel} to be validated.
	...
	public static void validateRow(ValueDateTupel[] valueDateTupels) {
		...
		if (Double.isNaN(values[0].doubleValue())) {
			...
			/*
			 * If the size of a set of all values is 1 then it contains only this one value in all elements. If this
			 * value is Double.NaN, no values were set but Double.NaN.
			 */
	...
	/**
	 * Validates the given Array of {@link Rule}. Must meet the following specifications:
	 * <ul>
	 * <li>Must not be null.</li>
	...
	/**
	 * Validates if the rules in the given array of {@link Rule} all share the given base scale.
	 * 
	...
	/**
	 * Validates the given array of {@link Rule} against the given {@link BaseValue}.
	 * <ul>
	 * <li>All rules must have the given {@link BaseValue} as their own {@link BaseValue}.</li>
	 * </ul>
	 * 
	 * @param  rules                    {@code Rule[]} The array of {@link Rule} to be validated.
	...
	public static void validateRulesVsBaseValue(Rule[] rules, BaseValue baseValue) {
		for (int i = 0; i < rules.length; i++)
			if (!rules[i].getBaseValue().equals(baseValue))
				throw new IllegalArgumentException("The base value of all rules must be equal to given base value"
				        + " but the rule at position " + i + " does not comply.");
	}

	/**
	 * Validates the given time window values. Values are needed for checking of containment. Must meet the following
	 * specifications:
	 * <ul>
	 ...
	 * </ul>
	 * 
	 * @param  startOfTimeWindow        {@link LocalDateTime} The start of Time window to be checked.
	 * @param  endOfTimeWindow          {@link LocalDateTime} The end of Time window to be checked.
	 * @param  values                   {@code ValueDateTupel[]} The array of {@link ValueDateTupel} to be checked.
	 * @throws IllegalArgumentException if any of the above specifications are not met.
	 */
	...
	/**
	 * Validates the given array of ValueDateTupel. The given array must fulfill the following specifications:
	 * <ul>
	 * <li>Must not be null</li>
	 ...
	 * @throws IllegalArgumentException if the given array does not meet the above specifications.
	 */
	...
	/**
	 * Validates the given variations array of {@link Rule} against the given {@link LocalDateTime} values for
	 * startOfReferenceWindow and endOfReferenceWindow and the given {@link BaseValue}. Must fulfill the following
	 * specifications:
	 * <ul>
	 * <li>Must not be of length greater than 3.</li>
	 * <li>Must not be of length 0.</li>
	 * <li>Elements must not be null.</li>
	 * <li>Elements' {@link Rule#getStartOfReferenceWindow()} must equal the given startOfReferenceWindow.</li>
	 * <li>Elements' {@link Rule#getEndOfReferenceWindow()} must equal the given endOfReferenceWindow.</li>
	 * <li>Must pass {@link #validateRulesVsBaseValue(Rule[], BaseValue)}.</li>
	 * </ul>
	 * 
	 * @param variations             {@code Rule[]} An array of {@link Rule} representing a rules' variations.
	 * @param startOfReferenceWindow {@link LocalDateTime} The start of reference window of the {@link Rule} containing
	 *                               the given variations.
	 * @param endOfReferenceWindow   {@link LocalDateTime} The end of reference window of the {@link Rule} containing
	 *                               the given variations.
	 * @param baseValue              {@link BaseValue} The base value of the {@link Rule} containing the given
	 *                               variations.
	 */
	public static void validateVariations(Rule[] variations, LocalDateTime startOfReferenceWindow,
		...
		for (int i = 0; i < variations.length; i++) {
			...
			if (!variations[i].getStartOfReferenceWindow().equals(startOfReferenceWindow)) {
				throw new IllegalArgumentException("The given reference window does not match the variation's at"
				        + " position " + i + ". The given start of reference window is different.");
			}
			if (!variations[i].getEndOfReferenceWindow().equals(endOfReferenceWindow)) {
				throw new IllegalArgumentException("The given reference window does not match the variation's at"
				        + " position " + i + ". The given end of reference window is different.");
			}
		}
