...
class UtilTest {
	...
	/**
	 * Test method for {@link Util#getPositionFromForecast(double)}.
	 */
	@Test
	void testGetPositionFromForecast() {
		final String LONG = "Long";
		final String SHORT = "Short";
		final String HOLD = "Hold";

		String actualValueLong = Util.getPositionFromForecast(5);
		String actualValueShort = Util.getPositionFromForecast(-10);
		String actualValueHold = Util.getPositionFromForecast(0);

		assertEquals(LONG, actualValueLong, "Position literal for positive forecasts is not inferred.");
		assertEquals(SHORT, actualValueShort, "Position literal for negative forecasts is not inferred.");
		assertEquals(HOLD, actualValueHold, "Position literal for forecasts of 0 is not inferred.");
	}
}